...
import { CheckCircle2, Download, FileText, Plus, Trash2, Upload, X } from 'lucide-react';
...
export default function DogDocumentsPanel({ dogId, showToast }: Props) {
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      showToast('Документ додано. Очікує перевірки.', 'success');
      resetForm();
    } catch (error: any) {
        ...
        <div className="space-y-2 mb-3">
          {documents.map(document => (
            <div key={document.id} className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5">
              ...
              {document.isChecked && document.checkedAt && (
                <div className="text-xs text-gray-500 mt-1">
                  Перевірено {new Date(document.checkedAt).toLocaleDateString('uk-UA')}
                </div>
              )}
