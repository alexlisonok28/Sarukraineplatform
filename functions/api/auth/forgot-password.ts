...
function randomToken() {
  ...
  // Сам токен отправляется пользователю, но в базе мы храним только SHA-256 hash.
...
export const onRequestPost = async ({ request, env }: Ctx) => {
  try {
    ...
    // Таблица создаётся автоматически при первом использовании функции.
    // Поэтому отдельно запускать миграцию в Neon пользователю не нужно.
    ...
    // Не сообщаем, существует ли такой email. Это не даёт постороннему человеку
    // использовать форму восстановления для перебора зарегистрированных адресов.
    if (!user) {
      return json({
        success: true,
        message: 'Якщо цей email зареєстрований, інструкції буде надіслано на пошту.',
      });
    }
    ...
    // Старые неиспользованные ссылки этого пользователя сразу делаем недействительными.
    ...
    // Ссылка ведёт на тот же Pages-сайт. React увидит resetToken в query string
    // и автоматически откроет страницу задания нового пароля.
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827;max-width:600px;margin:auto">
        ...
        <p style="margin:28px 0">
          <a href="${resetUrl.toString()}" style="background:#007AFF;color:white;text-decoration:none;padding:12px 20px;border-radius:10px;display:inline-block">Створити новий пароль</a>
        </p>
    ...
    try {
      await sendSmtpMail(
        ...
        {
          to: user.email,
          subject: 'Відновлення пароля SAR Ukraine',
          html,
          fromName: 'SAR Ukraine',
        },
      );
    } catch (error) {
      // Если письмо не ушло, удаляем созданную ссылку: в базе не должно оставаться
      // действующего токена, которым пользователь фактически не располагает.
